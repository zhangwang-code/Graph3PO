...
AutoCreatCluster_map (void)
{
  ...
  //读取配置模板
  string filename = "scratch/CephSim/14158_cluster_map.csv";
  ...
  for (uint32_t vecI = 0; vecI < hostN_vec_v.size (); vecI++)
    {
      ...
      sprintf (filename_buf, "scratch/CephSim/cluster_map/%05d_cluster_map.csv", vecI);
...
void
CreateWorkLoad (string path = "scratch/CephSim/workload.csv")
{
