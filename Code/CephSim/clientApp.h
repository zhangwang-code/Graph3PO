...
void
ClientApp::ReadWorkLoadFile (string path = "scratch/CephSim/workload.csv")
{
...
void
ClientApp::ReadOSDWaitTimePreFile (string path = "scratch/CephSim/osdWaitTimePredict.csv")
{
  std::ifstream in (path);
  string line;
...
ClientApp::StartApplication (void) // 启动应用
{
  ...
  ReadWorkLoadFile ("scratch/CephSim/workload.csv");

#ifdef IS_USE_OSD_WAIT_TIME
  //读取osd等待时间的预测信息
  cout << "Read osdWaitTimePredict.csv" << endl;
  ReadOSDWaitTimePreFile ("scratch/CephSim/osdWaitTimePredict.csv");
#endif
